...
public abstract class Uploader implements MessageConsumer {
  ...
  public void message(String s) {
    ...
    if (notFoundError) {
      error = I18n.format(getString("the selected serial port {0} does not exist or your board is not connected"), s);
      return;
    }
    if (s.contains("Device is not responding")) {
      error = getString("Device is not responding, check the right serial port is selected or RESET the board right before exporting");
      return;
    }
    if (StringUtils.stringContainsOneOf(s, AVRDUDE_PROBLEMS)) {
      error = getString("Problem uploading to board.  See http://www.arduino.cc/en/Guide/Troubleshooting#upload for suggestions.");
      return;
    }
    if (s.contains("Expected signature")) {
      error = getString("Wrong microcontroller found.  Did you select the right board from the Tools > Board menu?");
      return;
    }
  }
}
