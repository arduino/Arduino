...
public class SerialUploader extends Uploader {
    ...
    if (doTouch) {
      try {
        ...
        if (before.contains(userSelectedUploadPort)) {
          if (verbose)
            System.out.println(
              I18n.format(getString("Forcing reset using 1200bps open/close on port {0}"), userSelectedUploadPort));
  ...
  private String waitForUploadPort(String uploadPort, List<String> before) throws InterruptedException, RunnerException {
    ...
    throw new RunnerException(getString("Couldn't find a Board on the selected port. Check that you have the correct port selected.  If it is correct, try pressing the board's reset button after initiating the upload."));
  }
  ...
  public boolean uploadUsingProgrammer(String buildPath, String className) throws Exception {
    ...
    if (programmerPrefs == null)
      throw new RunnerException(
          getString("Please select a programmer from Tools->Programmer menu"));
  ...
  public boolean burnBootloader() throws Exception {
    ...
    if (tool.contains(":")) {
      ...
      if (toolPrefs.size() == 0)
        throw new RunnerException(I18n.format(getString("Could not find tool {0} from package {1}"), tool, split[0]));
    }
    toolPrefs.putAll(targetPlatform.getTool(tool));
    if (toolPrefs.size() == 0)
      throw new RunnerException(I18n.format(getString("Could not find tool {0}"), tool));
