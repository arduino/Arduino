  ...
  public LegacyTargetPlatform(String _name, File _folder, TargetPackage parent)
    ...
    if (!boardsFile.exists() || !boardsFile.canRead())
      throw new TargetPlatformException(
          format(getString("Could not find boards.txt in {0}. Is it pre-1.5?"),
    ...
    try {
      ...
      throw new TargetPlatformException(format(getString("Error loading {0}"),
    ...
    try {
      ...
      throw new TargetPlatformException(
          format(getString("Error loading {0}"), platformsFile.getAbsolutePath()), e);
    }
    ...
    try {
      ...
      throw new TargetPlatformException(
          format(getString("Error loading {0}"), localPlatformsFile.getAbsolutePath()), e);
    }
  ...
  private void rewriteKeysOfOldPlatformsTxtAndWarnAboutIt() throws IOException {
    ...
    for (Map.Entry<String, String> entry : oldProps.entrySet()) {
      ...
      if (preferences.containsKey(preferencesKey) && entry.getValue().equals(preferences.get(preferencesKey))) {
        System.err.println(I18n.format(getString("Warning: platform.txt from core '{0}' contains deprecated {1}, automatically converted to {2}. Consider upgrading this core."), platformName, preferencesKey + "=" + entry.getValue(), preferencesKey + "=" + newProps.get(entry.getKey())));
    ...
    for (Map.Entry<String, String> entry : addedProps.entrySet()) {
      ...
      if (!preferences.subTree(keyToAddFirstLevel).isEmpty() && !preferences.subTree(keyToAddSecondLevel).isEmpty() && !preferences.containsKey(keyToAdd)) {
        System.err.println(I18n.format(getString("Warning: platform.txt from core '{0}' misses property {1}, automatically set to {2}. Consider upgrading this core."), platformName, keyToAdd, entry.getValue()));
