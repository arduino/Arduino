...
public class CommandlineParser {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (a != null) {
        if (action != ACTION.GUI && action != ACTION.NOOP) {
          ...
          String mess = I18n.format(getString("Can only pass one of: {0}"), PApplet.join(valid, ", "));
        ...
        if (a == ACTION.INSTALL_BOARD) {
          i++;
          if (i >= args.length) {
            BaseNoGui.showError(null, I18n.format(getString("Argument required for {0}"), a.value), 3);
          }
      ...
      if (args[i].equals("--board")) {
        i++;
        if (i >= args.length)
          BaseNoGui.showError(null, getString("Argument required for --board"), 3);
        if (action == ACTION.GUI)
          action = ACTION.NOOP;
        continue;
      }
      if (args[i].equals("--port")) {
        i++;
        if (i >= args.length)
          BaseNoGui.showError(null, getString("Argument required for --port"), 3);
      ...
      if (args[i].equals("--curdir")) {
        i++;
        if (i >= args.length)
          BaseNoGui.showError(null, getString("Argument required for --curdir"), 3);
      ...
      if (args[i].equals("--pref")) {
        i++;
        if (i >= args.length)
          BaseNoGui.showError(null, getString("Argument required for --pref"), 3);
      ...
      if (args[i].equals("--preferences-file")) {
        i++;
        if (i >= args.length)
          BaseNoGui.showError(null, getString("Argument required for --preferences-file"), 3);
        // Argument should be already processed by Base.main(...)
        continue;
      }
      if (args[i].startsWith("--"))
        BaseNoGui.showError(null, I18n.format(getString("unknown option: {0}"), args[i]), 3);
  ...
  private void checkAction() {
    if ((action == ACTION.UPLOAD || action == ACTION.VERIFY) && filenames.size() != 1)
      BaseNoGui.showError(null, getString("Must specify exactly one sketch file"), 3);

    if ((action == ACTION.NOOP || action == ACTION.GET_PREF) && filenames.size() != 0)
      BaseNoGui.showError(null, getString("Cannot specify any sketch files"), 3);

    if ((action != ACTION.UPLOAD && action != ACTION.VERIFY) && (doVerboseBuild || doVerboseUpload))
      BaseNoGui.showError(null, getString("--verbose, --verbose-upload and --verbose-build can only be used together with --verify or --upload"), 3);
  }
  ...
  private void processBoardArgument(String selectBoard) {
    ...
    if (split.length < 3) {
      BaseNoGui.showError(null, I18n.format(getString("{0}: Invalid board name, it should be of the form \"package:arch:board\" or \"package:arch:board:options\""), selectBoard), 3);
    }

    TargetPackage targetPackage = BaseNoGui.getTargetPackage(split[0]);
    if (targetPackage == null) {
      BaseNoGui.showError(null, I18n.format(getString("{0}: Unknown package"), split[0]), 3);
    }

    TargetPlatform targetPlatform = targetPackage.get(split[1]);
    if (targetPlatform == null) {
      BaseNoGui.showError(null, I18n.format(getString("{0}: Unknown architecture"), split[1]), 3);
    }

    TargetBoard targetBoard = targetPlatform.getBoard(split[2]);
    if (targetBoard == null) {
      BaseNoGui.showError(null, I18n.format(getString("{0}: Unknown board"), split[2]), 3);
    }
    ...
    if (split.length > 3) {
      ...
      for (String option : options) {
        ...
        if (keyValue.length != 2)
          BaseNoGui.showError(null, I18n.format(getString("{0}: Invalid option, should be of the form \"name=value\""), option, targetBoard.getId()), 3);
        String key = keyValue[0].trim();
        String value = keyValue[1].trim();

        if (!targetBoard.hasMenu(key))
          BaseNoGui.showError(null, I18n.format(getString("{0}: Invalid option for board \"{1}\""), key, targetBoard.getId()), 3);
        if (targetBoard.getMenuLabel(key, value) == null)
          BaseNoGui.showError(null, I18n.format(getString("{0}: Invalid option for \"{1}\" option for board \"{2}\""), value, key, targetBoard.getId()), 3);
  ...
  private void processPrefArgument(String arg) {
    ...
    if (split.length != 2 || split[0].isEmpty())
      BaseNoGui.showError(null, I18n.format(getString("{0}: Invalid argument to --pref, should be of the form \"pref=value\""), arg), 3);
