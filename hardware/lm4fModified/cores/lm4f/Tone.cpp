...
uint8_t getTimerInterrupt(uint32_t * timer) {

	if(timer >= TIMER4) {
    	return(INT_TIMER0A + timer_to_offset[timer] + (timer_to_ab[tone_timer] >> 3));
    }
	else if(timer == TIMER3){
		return (INT_TIMER3A + (timer_to_ab[tone_timer] >> 3));
	}
	else {
		return(INT_TIMER4A + (timer_to_offset[timer] - 3) + (timer_to_ab[tone_timer] >> 3));
    }

}

uint32_t * getTimerBase(uint32_t * timer) {

	if(timer >= WTIMER2) {
    	return (TIMER0_BASE + timer_to_offset[timer] - 8);
    }
    else {
    	return (TIMER0_BASE + timer_to_offset[timer]);
    }
...
ToneIntHandler(void)
{
	...
	ROM_TimerIntClear(timerBase, TIMER_TIMA_TIMEOUT << timer_to_ab[tone_timer]);
    if(g_duration >= 0) {
	    if(--g_duration == 0){
	    	ROM_IntDisable(getTimerInterrupt(tone_timer));
			ROM_TimerIntDisable(tone_timer,TIMER_TIMA_TIMEOUT << timer_to_ab[tone_timer]);
			ROM_TimerIntClear(tone_timer,TIMER_TIMA_TIMEOUT << timer_to_ab[tone_timer]);
			tone_state = 0;
        }
    }
}
...
void tone(uint8_t _pin, unsigned int frequency, unsigned long duration)
{
  ...
  if (tone_state == 0 || _pin == current_pin) {
      ...
      ROM_TimerIntEnable(timerBase, TIMER_TIMA_TIMEOUT << timer_to_ab[tone_timer]);
...
void noTone(uint8_t _pin)
{
	...
	ROM_TimerIntDisable(timerBase,TIMER_TIMA_TIMEOUT << timer_to_ab[timer]);
	ROM_TimerIntClear(timerBase,TIMER_TIMA_TIMEOUT << timer_to_ab[timer]);
	tone_state = 0;
	g_duration = 0;
}
