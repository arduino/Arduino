...
public class Sketch {
  ...
  private SketchCodeDoc currentCodeDoc;
  ...
  private SketchCodeDoc[] codeDocs;
  ...
  protected void load() throws IOException {
      ...
      for (String extension : extensions) {
        if (base.toLowerCase().endsWith("." + extension)) {
          ...
          if (Sketch.isSanitaryName(base)) {
            codeDocs[codeCount++] =
              new SketchCodeDoc(new File(folder, filename));
          } else {
    ...
    codeDocs = (SketchCodeDoc[]) PApplet.subset(codeDocs, 0, codeCount);
  ...
  protected void insertCode(SketchCode newCode) {
    ...
    //if (codeCount == code.length) {
      codeDocs = (SketchCodeDoc[]) PApplet.append(codeDocs, newCode);
    codeCount++;
    //}
  ...
  protected void sortCode() {
    ...
    for (int i = 1; i < codeCount; i++) {
      ...
      if (who != i) {  // swap with someone if changes made
        SketchCodeDoc temp = codeDocs[who];
  ...
  protected void removeCode(SketchCode which) {
    ...
    for (int i = 0; i < codeCount; i++) {
      if (codeDocs[i].getCode() == which) {
        ...
        codeCount--;
        codeDocs = (SketchCodeDoc[]) PApplet.shorten(codeDocs);
        return;
      }
    }
...
//    }
    ...
    int bigCount = 0;
    for (SketchCodeDoc scd : codeDocs) {
