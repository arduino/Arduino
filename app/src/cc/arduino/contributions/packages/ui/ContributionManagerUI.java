...
public class ContributionManagerUI extends InstallerJDialog {
  ...
  public ContributionManagerUI(Frame parent, Platform platform) {
    super(parent, getString("Boards Manager"), Dialog.ModalityType.APPLICATION_MODAL, getString("Unable to reach Arduino.cc due to possible network issues."));
  ...
  public void onRemovePressed(final ContributedPlatform platform, boolean showWarning) {
    ...
    if (showWarning) {
      int chosenOption = JOptionPane.showConfirmDialog(this, I18n.format(getString("Do you want to remove {0}?\nIf you do so you won't be able to use {0} any more."), platform.getName()), getString("Please confirm boards deletion"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
